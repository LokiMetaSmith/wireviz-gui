...
class AddMateDialog(BaseFrame):
    def __init__(self, parent,
        ...
        r = 0
        tk.Label(self, text='Mate Connectors', **self._heading)\
            .grid(row=r, column=0, columnspan=2, sticky='ew')

        r += 1
        tk.Label(self, text='From:', **self._normal)\
            .grid(row=r, column=0, sticky='e')
        self._from_connector_cb = ttk.Combobox(self, values=list(self._harness.connectors.keys()))
        self._from_connector_cb.grid(row=r, column=1, sticky='ew')

        r += 1
        tk.Label(self, text='To:', **self._normal)\
            .grid(row=r, column=0, sticky='e')
        self._to_connector_cb = ttk.Combobox(self, values=list(self._harness.connectors.keys()))
        self._to_connector_cb.grid(row=r, column=1, sticky='ew')

        r += 1
        self._arrow_type_var = tk.StringVar(value='double')
        tk.Radiobutton(self, text='Whole Connector (==>)', variable=self._arrow_type_var, value='double', command=self._update_arrow_directions)\
            .grid(row=r, column=0, sticky='w')
        tk.Radiobutton(self, text='Pin-to-Pin (-->)', variable=self._arrow_type_var, value='single', command=self._update_arrow_directions)\
    ...
    def _update_arrow_directions(self):
        ...
        for direction in directions:
            tk.Radiobutton(self._arrow_direction_frame, text=direction, variable=self._arrow_direction_var, value=direction)\
